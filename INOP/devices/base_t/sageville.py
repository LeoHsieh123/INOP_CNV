...
class Sageville(devices.common.ethspy_commands.EthSpyCommands):
    """This class overrides and provides methods to control Sageville."""

    def __init__(self, port):
        """Initialize a Sageville object."""
        self.phy_type = 'SGVL'

        super(Sageville, self).__init__(port)

        self.codename = 'Sageville'
        self.interface = 'base_t'
        self.highest_speed = '10G'
        self.snr_type = ['SNR-OP-MARGIN', 'SNR-MIN-OP-MARGIN']
    ...
    def ethspy_lane_get_status(self, lane):
        """Lane status is unavailable for Sageville.
    ...
    def ethspy_rx_get_snr(self, abort, lane=0, ):
        """Get the SNR-OP-MARGIN from Sageville and return it.
        ...
        try:
            r = json.loads(
                self.port.execute(
                    'hostCommand ETHSPY RX GET-STATUS PHY:{} {}'.format(
                        self.phy_type, lane
                    )
                )
            )
    ...
    def ethspy_tx_get_status(self, lane):
        """The Tx status is unavailable for Sageville.
    ...
    def set_base_t_auto_neg(self, speeds=('100BASETX', '1GBASET', '2.5GBASET', '5GBASET', '10GBASET')):
        speed_list = ['100BASETX', '1GBASET', '2.5GBASET', '5GBASET', '10GBASET']

        self.port.execute(
            f'hostCommand ETHSPY LINK ENABLE-LINK-MANAGEMENT PHY:SGVL EN:FALSE'
        )

        for spd in speed_list:
            self.port.execute('hostCommand ETHSPY LINK RESET-CAPABILITY PHY:SGVL SIDE:LINE {}'.format(spd))
        
        for new_spd in speeds:
            self.port.execute('hostCommand ETHSPY LINK SET-CAPABILITY PHY:SGVL SIDE:LINE {}'.format(new_spd))

    def force_base_t_speed(self, speed):
        command = f'hostCommand ETHSPY LINK SET-CAPABILITY PHY:SGVL SIDE:LINE SPD:{speed[0:-5]}'
