    ...
    def set_base_t_auto_neg(self, speeds=('100M', '1G', '2.5G', '5G', '10G')):
        ...
        for new_spd in speeds:
            if new_spd == '2.5GBASET':
                self.port.execute(f'hostCommand ETHSPY LINK SET-CAPABILITY PHY:LKV SIDE:LINE SPD:2P5GIG_FD')
                time.sleep(10)
                self.port.execute(f'hostCommand ETHSPY LINK SET-CAPABILITY PHY:LKV SIDE:LINE SPD:2P5GIG_FD')

            elif new_spd == '5GBASET':
                self.port.execute(f'hostCommand ETHSPY LINK SET-CAPABILITY PHY:LKV SIDE:LINE SPD:5GIG_FD')
                time.sleep(10)
                self.port.execute(f'hostCommand ETHSPY LINK SET-CAPABILITY PHY:LKV SIDE:LINE SPD:5GIG_FD')
