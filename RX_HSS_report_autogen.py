...
def xlsx_general_format(wb,worksheet, start_row, start_column):
    ws = wb[worksheet]
...
def BER_p_f_format(wb,start_row, start_column):
    ws = wb["BER"]
    for i in range(start_row,ws.max_row+1):
...
def BER_error(wb,start_row, start_column):
    ws = wb["BER"]
    for i in range(start_row,ws.max_row+1):
...
def BER_CL(wb,start_row, start_column):
    ws = wb["BER"]
    for i in range(start_row,ws.max_row+1):
...
def BER_speed(wb,start_row, start_column):
    ws = wb["BER"]
    for i in range(start_row,ws.max_row+1):
    ...
    for i in range(start_row+1,ws.max_row+1):
        ...
        for j in range(iteration):
            ...
            if data["DUT"]["INFO"]["CODENAME"] == 'Connorsville' or data["DUT"]["INFO"]["CODENAME"] == 'Granite Rapids-D' :
                ...
                cell_value = ws.cell(row=i,column=start_column+j).value
                if cell_value == '1000M':
                    cell_value = '1G'

                if cell_value == test_speed_selection:
                    ws.cell(row=i,column=start_column+j).font = Font(name='Calibri',size=11,bold=True,color='000000')
                    ws.cell(row=i,column=start_column+j).fill = PatternFill(start_color="FFFFFF", fill_type="solid")
                elif cell_value != test_speed_selection:
...
def build_report(data,files,report_name,file_num):
    # list DUT/LP-INFO
    dut_info = [];lp_info=[]
    # list TTL
    ...
    ttl_top_eye_height_2 = []; ttl_top_eye_height_3=[];
    ttl_middle_eye_height_2 = []; ttl_middle_eye_height_3=[];
    ttl_bottom_eye_height_2 = []; ttl_bottom_eye_height_3=[];
    ...
    if data["DUT"]["INFO"]["CODENAME"] == 'Granite Rapids-D':
        """
        if test_cofig.speed == '100GBASE-CR2':
        ...
        elif test_cofig.speed == '200GBASE-CR4':
            #TTL_column = {"CONFIG":ttl_config, "TTL":ttl, "RETRY-ATTEMPTS":ttl_retry, "LINK-MODE":ttl_speed, "FEC-MODE":ttl_fec,"EYE-HEIGHT L0":ttl_eye_height_0,"EYE-HEIGHT L1":ttl_eye_height_1,"EYE-HEIGHT L2":ttl_eye_height_2,"EYE-HEIGHT L3":ttl_eye_height_3}
            TTL_column = {"CONFIG":ttl_config, "TTL":ttl, "RETRY-ATTEMPTS":ttl_retry, "LINK-MODE":ttl_speed, "FEC-MODE":ttl_fec,"TOP-EYE-HEIGHT L0":ttl_top_eye_height_0,"TOP-EYE-HEIGHT L1":ttl_top_eye_height_1,"TOP-EYE-HEIGHT L2":ttl_top_eye_height_2,"TOP-EYE-HEIGHT L3":ttl_top_eye_height_3,"MIDDLE-EYE-HEIGHT L0":ttl_middle_eye_height_0,"MIDDLE-EYE-HEIGHT L1":ttl_middle_eye_height_1,"MIDDLE-EYE-HEIGHT L2":ttl_middle_eye_height_2,"MIDDLE-EYE-HEIGHT L3":ttl_middle_eye_height_3,"BOTTOM-EYE-HEIGHT L0":ttl_bottom_eye_height_0,"BOTTOM-EYE-HEIGHT L1":ttl_bottom_eye_height_1,"BOTTOM-EYE-HEIGHT L2":ttl_bottom_eye_height_2,"BOTTOM-EYE-HEIGHT L3":ttl_bottom_eye_height_3}        
        elif test_cofig.speed == '100GBASE-CR4':
            #TTL_column = {"CONFIG":ttl_config, "TTL":ttl, "RETRY-ATTEMPTS":ttl_retry, "LINK-MODE":ttl_speed, "FEC-MODE":ttl_fec,"EYE-HEIGHT L0":ttl_eye_height_0,"EYE-HEIGHT L1":ttl_eye_height_1,"EYE-HEIGHT L2":ttl_eye_height_2,"EYE-HEIGHT L3":ttl_eye_height_3}
            TTL_column = {"CONFIG":ttl_config, "TTL":ttl, "RETRY-ATTEMPTS":ttl_retry, "LINK-MODE":ttl_speed, "FEC-MODE":ttl_fec,"EYE-HEIGHT L0":ttl_eye_height_0,"EYE-HEIGHT L1":ttl_eye_height_1,"EYE-HEIGHT L2":ttl_eye_height_2,"EYE-HEIGHT L3":ttl_eye_height_3}
        elif test_cofig.speed == '50GBASE-CR2':
            #TTL_column = {"CONFIG":ttl_config, "TTL":ttl, "RETRY-ATTEMPTS":ttl_retry, "LINK-MODE":ttl_speed, "FEC-MODE":ttl_fec,"EYE-HEIGHT L0":ttl_eye_height_0,"EYE-HEIGHT L1":ttl_eye_height_1,"EYE-HEIGHT L2":ttl_eye_height_2,"EYE-HEIGHT L3":ttl_eye_height_3}
            TTL_column = {"CONFIG":ttl_config, "TTL":ttl, "RETRY-ATTEMPTS":ttl_retry, "LINK-MODE":ttl_speed, "FEC-MODE":ttl_fec,"EYE-HEIGHT L0":ttl_eye_height_0,"EYE-HEIGHT L1":ttl_eye_height_1}
        elif test_cofig.speed == '50GBASE-CR1':
            #TTL_column = {"CONFIG":ttl_config, "TTL":ttl, "RETRY-ATTEMPTS":ttl_retry, "LINK-MODE":ttl_speed, "FEC-MODE":ttl_fec,"EYE-HEIGHT L0":ttl_eye_height_0,"EYE-HEIGHT L1":ttl_eye_height_1,"EYE-HEIGHT L2":ttl_eye_height_2,"EYE-HEIGHT L3":ttl_eye_height_3}
            TTL_column = {"CONFIG":ttl_config, "TTL":ttl, "RETRY-ATTEMPTS":ttl_retry, "LINK-MODE":ttl_speed, "FEC-MODE":ttl_fec,"TOP-EYE-HEIGHT L0":ttl_top_eye_height_0,"MIDDLE-EYE-HEIGHT L0":ttl_middle_eye_height_0,"BOTTOM-EYE-HEIGHT L0":ttl_bottom_eye_height_0}
        elif test_cofig.speed == '25GBASE-CR1':
            #TTL_column = {"CONFIG":ttl_config, "TTL":ttl, "RETRY-ATTEMPTS":ttl_retry, "LINK-MODE":ttl_speed, "FEC-MODE":ttl_fec,"EYE-HEIGHT L0":ttl_eye_height_0,"EYE-HEIGHT L1":ttl_eye_height_1,"EYE-HEIGHT L2":ttl_eye_height_2,"EYE-HEIGHT L3":ttl_eye_height_3}
        ...
        "CONFIG": ttl_config,
        "TTL": ttl,
        "RETRY-ATTEMPTS": ttl_retry,
        "LINK-MODE": ttl_speed,
        "FEC-MODE": ttl_fec
    }
        speed_map = {
        "100GBASET-CR2": [
            ("TOP-EYE-HEIGHT", [ttl_top_eye_height_0, ttl_top_eye_height_1]),
            ("MIDDLE-EYE-HEIGHT", [ttl_middle_eye_height_0, ttl_middle_eye_height_1]),
            ("BOTTOM-EYE-HEIGHT", [ttl_bottom_eye_height_0, ttl_bottom_eye_height_1]),
        ],
        "200GBASET-CR4": [
            ("TOP-EYE-HEIGHT", [ttl_top_eye_height_0, ttl_top_eye_height_1, ttl_top_eye_height_2, ttl_top_eye_height_3]),
            ("MIDDLE-EYE-HEIGHT", [ttl_middle_eye_height_0, ttl_middle_eye_height_1, ttl_middle_eye_height_2, ttl_middle_eye_height_3]),
            ("BOTTOM-EYE-HEIGHT", [ttl_bottom_eye_height_0, ttl_bottom_eye_height_1, ttl_bottom_eye_height_2, ttl_bottom_eye_height_3]),
        ],
        "100GBASET-CR4": [
            ("EYE-HEIGHT", [ttl_eye_height_0, ttl_eye_height_1, ttl_eye_height_2, ttl_eye_height_3]),
        ],
        "50GBASET-CR2": [
            ("EYE-HEIGHT", [ttl_eye_height_0, ttl_eye_height_1]),
        ],
        "50GBASET-CR1": [
            ("TOP-EYE-HEIGHT", [ttl_top_eye_height_0]),
            ("MIDDLE-EYE-HEIGHT", [ttl_middle_eye_height_0]),
            ("BOTTOM-EYE-HEIGHT", [ttl_bottom_eye_height_0]),
        ],
        "25GBASET-CR1": [
            ("EYE-HEIGHT", [ttl_eye_height_0]),
        ]
    }
        speed = test_cofig.speed
        if speed in speed_map:
            for name, values in speed_map[speed]:
                for idx, val in enumerate(values):
                    TTL_column[f"{name} L{idx}"] = val
...
#        print(ttl_config)
        ...
        # TTL data
            if test_cofig.link_iterations !='0':
                for j in TTL_column:
                    ...
                    elif j == "EYE-HEIGHT L0":
                        TTL_column[j].extend(data["DUT"]["LANE"]["0"]["EYE-HEIGHT"])

                    elif j == "EYE-HEIGHT L1":
                        TTL_column[j].extend(data["DUT"]["LANE"]["1"]["EYE-HEIGHT"])

                    elif j == "EYE-HEIGHT L2":
                        TTL_column[j].extend(data["DUT"]["LANE"]["2"]["EYE-HEIGHT"])

                    elif j == "EYE-HEIGHT L3":
                        TTL_column[j].extend(data["DUT"]["LANE"]["3"]["EYE-HEIGHT"])
                    ...
                    elif j == "TOP-EYE-HEIGHT L2":
                        TTL_column[j].extend(data["DUT"]["LANE"]["2"]["TOP-EYE-HEIGHT"])
                        
                    elif j == "MIDDLE-EYE-HEIGHT L2":
                        TTL_column[j].extend(data["DUT"]["LANE"]["2"]["MIDDLE-EYE-HEIGHT"])
                        
                    elif j == "BOTTOM-EYE-HEIGHT L2":
                        TTL_column[j].extend(data["DUT"]["LANE"]["2"]["BOTTOM-EYE-HEIGHT"])
                    elif j == "TOP-EYE-HEIGHT L3":
                        TTL_column[j].extend(data["DUT"]["LANE"]["3"]["TOP-EYE-HEIGHT"])
                        
                    elif j == "MIDDLE-EYE-HEIGHT L3":
                        TTL_column[j].extend(data["DUT"]["LANE"]["3"]["MIDDLE-EYE-HEIGHT"])
                        
                    elif j == "BOTTOM-EYE-HEIGHT L3":
                        TTL_column[j].extend(data["DUT"]["LANE"]["3"]["BOTTOM-EYE-HEIGHT"])                    
    ...
    if test_cofig.link_iterations !='0':
        data_ttl = pd.DataFrame(config1(TTL_column))
    if test_cofig.ber_iterations !='0':
        data_ber = pd.DataFrame(config1(BER_column))
    #data_ttl = data_ttl._append(data_ttl.mean(axis=0),ignore_index=True)
    #data_ber = data_ber._append(empty_avg,ignore_index=True)

    writer = pd.ExcelWriter(report_name, engine='openpyxl') # pylint: disable=abstract-class-instantiated
    if test_cofig.link_iterations !='0':
        if test_cofig.ber_iterations !='0':
...
def beautify_report(report_name):
    wb = op.load_workbook(report_name)
    ...
    if test_cofig.link_iterations !='0':
        xlsx_general_format(wb,worksheet="TTL", start_row=1, start_column=1)
    if test_cofig.ber_iterations !='0':
        xlsx_general_format(wb,worksheet="BER", start_row=1, start_column=1)
    ...
    if test_cofig.ber_iterations !='0':
        BER_speed(wb,start_row=2, start_column=6)
        BER_error(wb,start_row=2, start_column=3)
        BER_CL(wb,start_row=2, start_column=5)
        BER_p_f_format(wb,start_row=2, start_column=7)
...
for idx, data in enumerate(all_data):
    report_name = raw_data_path + "/" + f"{raw_data_path.split('/')[-1]}_RX_report_{data["DUT"]["INFO"]["CODENAME"]}.xlsx"
    build_report(data,files,report_name,idx)
    beautify_report(report_name)
shutil.rmtree(copy_dir_path)
print("Done...(6/6)\n=============================")
